...
- `f_m`：测得的比力，即单位质量所受非引力合力
- `f_true`：真实比力，不是运动学加速度
- `w_m`：测得的角速度
- `b_a`、`b_g`：缓慢变化的 bias
- `n_a`、`n_g`：白噪声

物理上，在惯性系中有 `f = a - gamma`，其中 `gamma` 是地球引力加速度。
转换到随地球旋转的导航系后，惯导速度方程通常使用包含离心效应的有效重力
`g`，并显式包含地球自转、运输角速度和科氏项。比力、运动学加速度、引力和
有效重力不是同一个量，详细定义见文档 08。
...
工程实现通常使用 Joseph 形式 ``A P Aᵀ + K R Kᵀ``，
其中 ``A = I - K H``，以获得更好的数值稳定性。
